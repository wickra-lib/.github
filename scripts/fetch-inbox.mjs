 ...
 *      is neither a bot, nor an automation account, nor an org member.
 *
...
const members = new Set((await paged(`https://api.github.com/orgs/${ORG}/members?per_page=100`)).map((m) => m.login))
const outsider = (user) => !isBot(user) && !members.has(user.login)
...
for (const repo of repos) {
  ...
  for (const issue of issues) {
    if (issue.pull_request || !outsider(issue.user)) continue
...
lines.push(`Last scan: **${scannedAt}** over ${repos.length} repositories. Both lists hold only what is open now.`, '')
...
await writeFile('inbox/state.json', JSON.stringify({ scanned_at: scannedAt, repos: repos.length, dependabot, waiting, people }, null, 2) + '\n')
console.log(`inbox: ${dependabot.length} Dependabot PRs with finished checks (${waiting} waiting), ${people.length} items by people outside the org, ${repos.length} repositories`)
