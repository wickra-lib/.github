 ...
 *
 * Two lists, rewritten from scratch on every run so they hold only what is
 ...
 *      collaborator of the repository (the item's author_association, which
 *      also covers private org memberships the members endpoint hides).
 *   3. Open security alerts -- code scanning, Dependabot and secret scanning,
 *      one row per alert. Alert endpoints need a token with security_events
 *      read on the repository; the workflow token has that for this
 *      repository only, so INBOX_TOKEN (a fine-grained token with read access
 *      to code scanning, Dependabot and secret scanning alerts on the org's
 *      repositories) is used when set. Repositories the token cannot read are
 *      named, not silently counted as clean.
 *
...
const ALERTS_TOKEN = process.env.INBOX_TOKEN || process.env.GH_TOKEN
const ALERTS_TOKEN_NAME = process.env.INBOX_TOKEN ? 'INBOX_TOKEN' : 'GITHUB_TOKEN'

const sleep = (ms) => new Promise((r) => setTimeout(r, ms))

async function request(url, token = process.env.GH_TOKEN) {
  let reason = null
...
async function paged(url, token) {
  ...
  let next = url
  while (next) {
    const res = await request(next, token)
    if (!res) break
...
async function alertsOf(repo) {
  ...
  const pull = async (kind, path) => {
    try {
      return await paged(`https://api.github.com/repos/${ORG}/${repo}/${path}?state=open&per_page=100`, ALERTS_TOKEN)
    } catch (err) {
...
lines.push(`Code scanning, Dependabot and secret scanning alerts, read with ${ALERTS_TOKEN_NAME}.`, '')
if (unreadable.length) {
  const named = unreadable.map((u) => `\`${u.repo}\` (${u.kinds.join(', ')})`).join(', ')
  lines.push(`Not readable with this token, so not counted: ${named}. Set INBOX_TOKEN to a token with read access to the three alert lists on the org's repositories.`, '')
}
if (alerts.length) {
